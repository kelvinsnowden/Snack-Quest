...
export const AchievementsView: React.FC<AchievementsViewProps> = ({ creator }) => {
  const current = calculateCreatorTier(creator.lifetime_earnings, creator.total_conversions);
  const progressPct = Math.min(100, Math.round((creator.lifetime_earnings / current.nextThresholdKes) * 100));
      ...
      <section className="bg-creator-surface border border-creator-border rounded-creator-card-lg p-6 sm:p-8">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between text-creator-caption text-creator-ink-muted mb-2">
            <span>{formatKes(creator.lifetime_earnings)} earned</span>
            <span>Next tier at {formatKes(current.nextThresholdKes)}</span>
          </div>
          <div
            className="h-2 rounded-full bg-creator-surface-hover overflow-hidden"
            role="progressbar"
