...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`h-screen sticky top-0 z-40 bg-[#09090B] border-r border-[#27272A] text-[#FAFAFA] flex flex-col transition-all duration-300 select-none ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Brand Header */}
      <div className="p-6 border-b border-[#27272A] flex items-center justify-between">
        ...
        <button
          ...
          className="p-1.5 rounded-md text-[#71717A] hover:text-white hover:bg-[#18181B] transition-colors cursor-pointer hidden sm:block"
          ...
          <nav className="space-y-1">
            {upcomingNavItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  title={collapsed ? `${item.label} (${item.prompt})` : undefined}
