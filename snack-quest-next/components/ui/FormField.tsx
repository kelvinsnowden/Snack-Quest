...
// Ported verbatim from the current Vite app, which uses `any` to
// destructure the polymorphic input/textarea/select rest props. The
// Next.js project's ESLint config is stricter than the source
// project's; disabling here rather than changing the signature, per
// the "port verbatim, fix import paths only" instruction.
...
export const FormField: React.FC<FormFieldProps> = (props) => {
  ...
  const fieldClasses = `w-full bg-creator-canvas border rounded-creator-control px-3.5 py-2.5 text-creator-body text-creator-ink placeholder:text-creator-ink-faint transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-creator-brand/40 disabled:opacity-50 disabled:cursor-not-allowed ${
    error
      ? 'border-creator-danger focus-visible:ring-creator-danger/40'
      : 'border-creator-border focus:border-creator-brand'
  }`;
  ...
  return (
    <div className={containerClassName}>
      <label
        htmlFor={id}
        className="text-creator-caption text-creator-ink-muted mb-1.5 block font-semibold"
      >
        {label}
        {required && (
      ...
      {error ? (
        <p
          id={errorId}
          className="text-creator-caption text-creator-danger mt-1.5"
        >
          {error}
        </p>
      ) : helpText ? (
        <p
          id={helpId}
          className="text-creator-caption text-creator-ink-faint mt-1.5"
        >
          {helpText}
        </p>
      ) : null}
