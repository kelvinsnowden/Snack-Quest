...
/**
 * Real staff sign-in (§ Admin auth foundation): Firebase Client SDK
 * checks the password, this component never sees or validates it
 * itself. The resulting ID token is exchanged for an httpOnly session
 * cookie via `POST /api/auth/session` — a Firebase Auth success alone
 * is not enough to reach `/admin`, since that route also confirms the
 * uid is provisioned staff (see `services/staffAuthService.ts`).
 */
