...
export interface PayoutGateway {
  initiateB2CPayment(input: {
    ...
    occasion?: string;
    /**
     * Generated and durably persisted by the caller (`WithdrawalService`)
     * *before* this call is ever made — never generated by the Gateway
     * itself. Daraja's B2C v3 contract requires the caller to supply
     * this as a real request field (not just read it back from the
     * response, as the older v1 contract allowed); it's also the
     * correlation id every later step depends on — the async
     * `Result.OriginatorConversationID` echoes it back, and a stuck
     * withdrawal's reconciliation Transaction Status Query is keyed off
     * it. Persisting it before this network call is what makes a crash
     * between "Daraja accepted the request" and "we recorded the
     * response" recoverable instead of silently losing track of a real
     * payout.
     */
    originatorConversationId: string;
   ...
   * `PaymentGateway.queryStkStatus`, for a withdrawal whose B2C
   * ResultURL never arrived. Unlike the STK query (synchronous —
   * Safaricom answers in the HTTP response itself), Transaction Status
   * Query is asynchronous like B2C itself: this call only returns
   * Safaricom's acknowledgement that the *query* was accepted; the
   * actual answer arrives later at a dedicated ResultURL, parsed by
   * `verifyTransactionStatusResult`.
   */
  queryTransactionStatus(input: {
    businessId: string;
    /** The original transaction's own `OriginatorConversationID` — the identifier we generated and persisted before the original B2C call, used here to ask Daraja about a transaction whose receipt number we may never have received. */
    originatorConversationId: string;
    remarks: string;
    occasion?: string;
...
/** The synchronous ack for a Transaction Status Query request itself — `originatorConversationId`/`conversationId` here identify the *query call*, not the original transaction being asked about. */
export interface TransactionStatusQueryAck {
  originatorConversationId: string;
  conversationId: string;
  responseCode: string;
  responseDescription: string;
...
/**
 * The async outcome of a Transaction Status Query, delivered to its own
 * dedicated ResultURL. `originatorConversationId` here echoes the
 * *query* request's own id (matched back to the withdrawal via
 * `Withdrawal.pendingStatusQueryOriginatorConversationId`), not the
 * original transaction's id.
 *
 * Deliberately conservative: `transactionStatus` is Safaricom's own
 * free-text status field for the *queried* transaction, read verbatim
 * and never interpreted as anything beyond an exact, case-sensitive
 * `'Completed'` match for "safe to mark paid" — every other value
 * (including ones this codebase doesn't recognize) must be treated as
 * still-ambiguous, per the same "never fabricate a successful payment"
 * rule the STK Push Query reconciliation sweep already follows.
 */
export interface TransactionStatusResult {
  ...
  resultCode: number;
  resultDesc: string;
  /** Safaricom's own status text for the *original* transaction, when the query itself succeeded (`resultCode === 0`) and a status parameter was present. Undefined otherwise — never guessed at. */
