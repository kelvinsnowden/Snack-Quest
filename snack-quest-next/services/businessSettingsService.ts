...
class BusinessSettingsService {
  ...
  private validate(patch: BusinessSettingsPatch): void {
    ...
    if (patch.currency !== undefined && !CURRENCY_PATTERN.test(patch.currency)) {
      throw new BusinessSettingsValidationError('"currency" must be a 3-letter ISO code, e.g. "KES".');
    }
    if (patch.whatsappPhoneNumberId !== undefined && patch.whatsappPhoneNumberId.trim().length === 0) {
      throw new BusinessSettingsValidationError('"whatsappPhoneNumberId" cannot be empty.');
    }
    if (patch.countyCoverage !== undefined) {
      if (!Array.isArray(patch.countyCoverage) || patch.countyCoverage.some((c) => typeof c !== 'string' || c.trim().length === 0)) {
        throw new BusinessSettingsValidationError('"countyCoverage" must be a list of non-empty county names.');
      }
    }
