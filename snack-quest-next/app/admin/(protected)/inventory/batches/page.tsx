...
export default async function AdminInventoryBatchesPage({
  ...
  const [{ batches, nextCursor }, products, suppliers] = await Promise.all([
    inventoryBatchRepository.listActive(session.businessId, { cursor }),
    packageRepository.listAllByBusiness(session.businessId),
    supplierRepository.listByBusiness(session.businessId),
  ]);
  const productNameById = new Map(
    products.map(({ id, data }) => [id, data.name]),
  );
  const supplierNameById = new Map(
    suppliers.map(({ id, data }) => [id, data.name]),
  );
        ...
        <Card className="overflow-hidden p-0">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-sm">
              ...
              <tbody>
                {batches.map(({ id, data }) => {
                  const status = classifyBatchExpiry(
                    data.expiresAt,
                    EXPIRING_SOON_DAYS,
                  );
                  return (
                    ...
                    >
                      <td className="text-foreground px-4 py-3 font-medium">
                        {productNameById.get(data.packageId) ?? data.packageId}
                      </td>
                      <td className="text-foreground px-4 py-3">
                        {supplierNameById.get(data.supplierId) ?? '—'}
                      </td>
                      <td className="text-foreground px-4 py-3 tabular-nums">
                        {data.quantityRemaining} / {data.quantityReceived}
                      </td>
                      <td className="text-foreground px-4 py-3 tabular-nums">
                        {formatKes(data.unitCostKes)}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <WriteOffBatchDialog
                          batchId={id}
                          productName={
                            productNameById.get(data.packageId) ??
                            data.packageId
                          }
      ...
      {nextCursor ? (
        <div className="flex justify-center">
          <Button asChild variant="outline">
            <Link href={`/admin/inventory/batches?cursor=${nextCursor}`}>
              Load more
            </Link>
          </Button>
        </div>
