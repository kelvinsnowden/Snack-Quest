...
export default async function AdminConversationsPage({
  ...
  const validStatus = STATUS_FILTERS.includes(status as ConversationStatus)
    ? (status as ConversationStatus)
    : undefined;

  const { conversations, nextCursor } =
    await conversationService.listConversations(session.businessId, {
      status: validStatus,
      cursor,
    });
  ...
  const agents = await Promise.all(
    conversations.map(({ data }) =>
      data.assignedAgentId
        ? userRepository.findById(data.assignedAgentId)
        : null,
    ),
  );
              ...
              <tbody>
                {conversations.map(({ id, data }, index) => (
                  ...
                  >
                    ...
                    <td className="text-foreground px-4 py-3 capitalize">
      ...
      {nextCursor ? (
        <div className="flex justify-center">
          <Button asChild variant="outline">
            <Link
              href={`/admin/conversations?${status ? `status=${status}&` : ''}cursor=${nextCursor}`}
            >
              Load more
            </Link>
          </Button>
