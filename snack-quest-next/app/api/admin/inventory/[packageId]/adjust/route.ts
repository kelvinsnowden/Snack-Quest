...
const VALID_REASONS: InventoryMovementReason[] = ['restock', 'correction', 'damaged', 'other'];
...
export async function POST(
  ...
  const { delta, reason, note } = (body ?? {}) as { delta?: unknown; reason?: unknown; note?: unknown };

  if (typeof delta !== 'number' || !Number.isInteger(delta) || delta === 0) {
    return Response.json({ error: '"delta" must be a non-zero integer.' }, { status: 400 });
  }
  if (typeof reason !== 'string' || !VALID_REASONS.includes(reason as InventoryMovementReason)) {
  ...
  try {
    ...
    if (error instanceof StockNotTrackedError || error instanceof InsufficientStockError) {
