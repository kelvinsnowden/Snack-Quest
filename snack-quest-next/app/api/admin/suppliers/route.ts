...
import { supplierService, InvalidSupplierInputError } from '@/services/supplierService';
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (typeof body.name !== 'string' || typeof body.contactName !== 'string' || typeof body.phone !== 'string') {
    return Response.json({ error: '"name", "contactName", and "phone" are required strings.' }, { status: 400 });
  }
  if (body.email !== undefined && body.email !== null && typeof body.email !== 'string') {
    return Response.json({ error: '"email" must be a string or null when provided.' }, { status: 400 });
  }
  ...
  try {
    ...
    await recordAuditLog(request, {
      ...
      entityType: 'supplier',
      entityId: supplierId,
      after: { name: body.name, contactName: body.contactName, phone: body.phone },
    });
    ...
    return Response.json({ error: error instanceof Error ? error.message : 'Could not create supplier' }, { status: 400 });
  }
}
