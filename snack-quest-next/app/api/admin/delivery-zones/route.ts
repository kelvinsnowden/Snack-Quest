...
export async function PATCH(request: Request): Promise<Response> {
  ...
  if (
    ...
    return Response.json(
      { error: '"zone", "shippingOrigin", "packageCategory", "courier" (strings) and "feeKes" (number) are required.' },
      { status: 400 },
    );
  }

  const key = { zone: body.zone, shippingOrigin: body.shippingOrigin, packageCategory: body.packageCategory, courier: body.courier };

  try {
    const { stationsUpdated } = await deliveryZoneService.setZoneFee(session.businessId, key, body.feeKes, session.uid);
    ...
    return Response.json({ error: error instanceof Error ? error.message : 'Could not update this zone.' }, { status: 400 });
  }
}
