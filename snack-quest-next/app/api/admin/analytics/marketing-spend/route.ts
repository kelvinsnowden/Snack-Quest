...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!isValidOptionalAmount(metaSpendKes)) {
    return Response.json({ error: '"metaSpendKes" must be a non-negative number when provided.' }, { status: 400 });
  }
  if (!isValidOptionalAmount(tiktokSpendKes)) {
    return Response.json({ error: '"tiktokSpendKes" must be a non-negative number when provided.' }, { status: 400 });
  }
