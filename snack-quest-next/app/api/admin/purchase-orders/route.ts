...
import { purchaseOrderService, InvalidPurchaseOrderInputError } from '@/services/purchaseOrderService';
...
function validateLineItems(raw: unknown): { error: string } | { lineItems: { packageId: string; quantity: number; unitCostKes: number }[] } {
  if (!Array.isArray(raw) || raw.length === 0) {
    return { error: '"lineItems" must be a non-empty array.' };
  }
  const lineItems: { packageId: string; quantity: number; unitCostKes: number }[] = [];
  for (const raw_item of raw) {
    ...
    lineItems.push({ packageId: item.packageId, quantity: item.quantity, unitCostKes: item.unitCostKes });
  }
  return { lineItems };
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (body.expectedDeliveryAt !== undefined && body.expectedDeliveryAt !== null && typeof body.expectedDeliveryAt !== 'string') {
    return Response.json({ error: '"expectedDeliveryAt" must be a date string or null when provided.' }, { status: 400 });
  }
  ...
  try {
    const purchaseOrderId = await purchaseOrderService.createPurchaseOrder(
      session.businessId,
      {
        ...
        expectedDeliveryAt: body.expectedDeliveryAt as string | null | undefined,
    ...
    await recordAuditLog(request, {
      ...
      after: { supplierId: body.supplierId, lineItemCount: lineItemsResult.lineItems.length },
    });
    ...
    return Response.json({ error: error instanceof Error ? error.message : 'Could not create purchase order' }, { status: 400 });
  }
}
