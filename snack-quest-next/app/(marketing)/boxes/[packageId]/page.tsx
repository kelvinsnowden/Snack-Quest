...
export default async function BoxDetailPage({ params }: { params: Promise<{ packageId: string }> }) {
  ...
  const [box, business] = await Promise.all([packageRepository.findById(businessId, packageId), getCurrentBusiness()]);
  if (!box || !box.isActive) {
    notFound();
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid animate-fade-in gap-10 lg:grid-cols-2 lg:items-start">
        ...
        <div>
          <h1 className="text-page-title font-bold tracking-tight text-foreground">{box.name}</h1>
          <p className="mt-3 text-2xl font-semibold text-foreground">{formatKes(box.priceKes)}</p>
          <p className="mt-4 text-subtitle text-muted-foreground">{box.description}</p>

          <div className="mt-8">
            <WhatsAppOrderButton whatsappCustomerNumber={business?.whatsappCustomerNumber ?? null} message={message} />
          </div>

          <p className="mt-4 text-sm text-muted-foreground">
            Door delivery in Nairobi or pickup station nationwide — we&apos;ll confirm delivery options once you message us.
          </p>
        </div>
      </div>
    </div>
