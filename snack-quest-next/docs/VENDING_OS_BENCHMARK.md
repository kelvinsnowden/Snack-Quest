...
| Manufacturer-agnostic adapter registry | EXISTING / KEEP, then EXTENDED — see `docs/HARDWARE_COMPATIBILITY_ARCHITECTURE.md` | `adapterRegistry.ts` throws `UnsupportedManufacturerError` for `other` rather than guessing at an undocumented API. `shengma` now resolves to an honest `ShengmaAdapter` stub (all capabilities `false`) instead of throwing, once the hardware-compatibility pivot needed something to show in the admin diagnostics panel — still zero guessed-at Shengma behaviour. |
