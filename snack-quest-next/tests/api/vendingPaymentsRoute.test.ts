...
const { authenticateDeviceMock, initiateMpesaPaymentMock, initiateCartPaymentMock, findByIdMock } = vi.hoisted(() => ({
  authenticateDeviceMock: vi.fn(),
  initiateMpesaPaymentMock: vi.fn(),
  initiateCartPaymentMock: vi.fn(),
  findByIdMock: vi.fn(),
}));
...
vi.mock('@/services/machineTransactionService', async () => {
  ...
  return {
    ...actual,
    machineTransactionService: { initiateMpesaPayment: initiateMpesaPaymentMock, initiateCartPayment: initiateCartPaymentMock, findById: findByIdMock },
  };
});
...
describe('POST /api/vending/payments — cart of slotIds', () => {
  it('calls initiateCartPayment, never initiateMpesaPayment, for a slotIds array', async () => {
    authenticateDeviceMock.mockResolvedValue({ ok: true, businessId: 'biz-1', machineId: 'm-1', credentialId: 'cred-1' });
    initiateCartPaymentMock.mockResolvedValue({
      checkoutRequestId: 'ws_CO_cart',
      merchantRequestId: 'mr-cart',
    ...
    const response = await paymentsPost(postRequest({ slotIds: ['A01', 'B01'], phoneNumber: '0712345678' }));

    expect(response.status).toBe(201);
    expect(initiateCartPaymentMock).toHaveBeenCalledWith(
      expect.objectContaining({ businessId: 'biz-1', machineId: 'm-1', slotIds: ['A01', 'B01'], phoneNumber: '254712345678' }),
    );
    expect(initiateMpesaPaymentMock).not.toHaveBeenCalled();
  ...
  it('400s an empty slotIds array without ever calling the service', async () => {
    authenticateDeviceMock.mockResolvedValue({ ok: true, businessId: 'biz-1', machineId: 'm-1', credentialId: 'cred-1' });
    const response = await paymentsPost(postRequest({ slotIds: [], phoneNumber: '0712345678' }));
    expect(response.status).toBe(400);
    expect(initiateCartPaymentMock).not.toHaveBeenCalled();
  });

  it('409s a cart where one slot is unavailable for sale', async () => {
    authenticateDeviceMock.mockResolvedValue({ ok: true, businessId: 'biz-1', machineId: 'm-1', credentialId: 'cred-1' });
    initiateCartPaymentMock.mockRejectedValue(new SlotUnavailableForSaleError('m-1', 'B01', 'out of stock'));
    const response = await paymentsPost(postRequest({ slotIds: ['A01', 'B01'], phoneNumber: '0712345678' }));
    expect(response.status).toBe(409);
  });
});
