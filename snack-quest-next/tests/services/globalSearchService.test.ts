...
beforeEach(async () => {
  await Promise.all(
    ['packages', 'suppliers', 'purchaseOrders', 'conversations', 'creatorProfiles', 'users'].map((name) =>
...
describe('GlobalSearchService.search', () => {
  ...
  it('finds a product by name', async () => {
    await packageRepository.create(
      { businessId: BUSINESS_ID, name: 'Starter Box', priceKes: 2500, description: '', isActive: true, imageUrl: null, stockCount: 10 },
      'staff-1',
    );
    await packageRepository.create(
      { businessId: BUSINESS_ID, name: 'Deluxe Box', priceKes: 5000, description: '', isActive: true, imageUrl: null, stockCount: 3 },
      'staff-1',
    );
    ...
    expect(response.results.some((r) => r.type === 'product' && r.title === 'Deluxe Box')).toBe(false);
  });

  it('finds inventory by product name, linked to the inventory page', async () => {
    await packageRepository.create(
      { businessId: BUSINESS_ID, name: 'Starter Box', priceKes: 2500, description: '', isActive: true, imageUrl: null, stockCount: 42 },
      'staff-1',
    );
    const response = await globalSearchService.search(BUSINESS_ID, 'starter');
    const inventoryHit = response.results.find((r) => r.type === 'inventory');
    expect(inventoryHit).toMatchObject({ title: 'Starter Box', href: '/admin/inventory', subtitle: '42 in stock' });
  });

  it('finds a supplier by name, contact name, or phone', async () => {
    await supplierRepository.create(
      { businessId: BUSINESS_ID, name: 'Coastal Snacks Ltd', contactName: 'Jane Wanjiru', phone: '254700000001', email: null, notes: '', isActive: true },
      'staff-1',
    );
    const byName = await globalSearchService.search(BUSINESS_ID, 'coastal');
    expect(byName.results.some((r) => r.type === 'supplier')).toBe(true);
    const byPhone = await globalSearchService.search(BUSINESS_ID, '254700000001');
  ...
  it('finds a purchase order by its resolved supplier name', async () => {
    const supplierId = await supplierRepository.create(
      { businessId: BUSINESS_ID, name: 'Coastal Snacks Ltd', contactName: 'Jane', phone: '254700000001', email: null, notes: '', isActive: true },
      'staff-1',
    );
  ...
  it('finds a conversation by phone number', async () => {
    await conversationRepository.create({ businessId: BUSINESS_ID, phoneNumber: '254712345678' });
    const response = await globalSearchService.search(BUSINESS_ID, '254712345678');
  ...
  it('finds a creator by their user display name', async () => {
    ...
    await userRepository.create(uid, { email: 'amina@example.com', roles: ['creator'], displayName: 'Amina Hassan', photoURL: null }, 'system');
  ...
  it('never leaks another business\'s data', async () => {
    await packageRepository.create(
      { businessId: OTHER_BUSINESS_ID, name: 'Rival Starter Box', priceKes: 2500, description: '', isActive: true, imageUrl: null, stockCount: 10 },
      'staff-1',
    );
    const response = await globalSearchService.search(BUSINESS_ID, 'rival starter');
    expect(response.results).toHaveLength(0);
  });
});
